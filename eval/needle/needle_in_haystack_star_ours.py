    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        with torch.no_grad():
            if self.args.prefilling_chunk_size is not None:
                if self.args.prefilling_chunk_size is not None:
                    past_key_values = []
                    chunk = prompt_input_ids[:, 0:self.args.prefilling_chunk_size]
                    output = self.model_to_test(
                        input_ids=chunk,
                        past_key_values=past_key_values,
                        use_cache=True,
                    )
                    sink_past_key_values = output.past_key_values

                    for i in range(
                            self.args.prefilling_chunk_size, prompt_input_ids.size(1), self.args.prefilling_chunk_size
                    ):
                        chunk = prompt_input_ids[:, i: i + self.args.prefilling_chunk_size]
                        output = self.model_to_test(
                            input_ids=chunk,
                            past_key_values=sink_past_key_values,
                            use_cache=True,
                        )
                        gen_past_key_values = output.past_key_values

                    #拼接cache
                    if past_key_values == None:
                        past_key_values = []
                        for l in len(sink_past_key_values):
                            k = gen_past_key_values[0]
                            v = gen_past_key_values[1]
                            kv = [k, v]
                            past_key_values.append(kv)

                    for l in len(sink_past_key_values):
                        k = torch.cat([past_key_values[l][0][:, :, :, :],
                                       gen_past_key_values[l][0][:, :, self.args.prefilling_chunk_size:, :].detach().clone()], dim=2)
                        v = torch.cat([past_key_values[l][1][:, :, :, :],
                                       gen_past_key_values[l][1][:, :, self.args.prefilling_chunk_size:, :].detach().clone()], dim=2)
                        kv = [k, v]
                        past_key_values[i] = kv
                    del gen_past_key_values
            else:
                output = self.model_to_test(
