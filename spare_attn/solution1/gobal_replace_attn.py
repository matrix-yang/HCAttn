...
@jit(nopython=True)
def get_head_repeat_index(mat, th=2, sink=8, recent=64):
    size=mat.shape
    ...
    mids_len=np.array([len(i) for i in mids])
    sort_indices = np.argsort(mids_len)
    ...
    radio=len(set(repeat_index_final))/size[-1]
    return repeat_index_final,radio
...
def get_clone_index(mat, th):
    ...
    for i in range(size[0]):
        mids,r=get_head_repeat_index(mat[i], th=th)
        #mids=get_index2(mat[i],th)
        repeat[i]=mids
        radios.append(r)
    return repeat,radios
