...
def decode_token_cpu(query_states_ori, key_states_ori, value_states_ori, sum_value, radio_bag):
    ...
    select_value_states=select_value_states.cpu()
    s = time.time()
    attn_weights=attn_weights.cpu()
    attn_output = torch.matmul(attn_weights, select_value_states)
    attn_output = attn_output.unsqueeze(0).transpose(1, 2)
    # print(attn_output.shape)
    attn_output=attn_output.to(key_states.device)
    e = time.time()
    #print(f'decode 1 token 1 layers use time {e - s}')
    radio_bag.append(e - s)
    return attn_output
